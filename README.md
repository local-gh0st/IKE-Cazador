...
# ike-cazador
tool for discovering and validating IKE Aggressive Mode on target hosts and capturing PSK hashes for offline cracking. Built for use during authorized PenTests, promise.

## What it does
this tool runs in two phases (but single command):
...
python3 ike-cazador.py targets.txt
python3 ike-cazador.py targets.txt wordlist.txt
python3 ike-cazador.py targets.txt wordlist.txt --conservative -p 4500
python3 ike-cazador.py --resume ike-cazador-results/20260414_161904 new-wordlist.txt
...
Options
Run python3 ike-cazador.py -h for the full list of flags including:
- timing controls
- interface selection
- conservative mode
- and more? 

help output also includes a "Notify Messages Cheat Sheet" explaining what each (tested and accounted for) IKE response code means.
---
